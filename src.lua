...
function SaveLoadSystem:CollectData()
    ...
    for id, obj in pairs(self.elementRegistry.Checkboxes) do
        if obj and obj.GetValue then
            data.Checkboxes[id] = obj:GetValue()
        end
    end
...
function SaveLoadSystem:ResetAll()
    ...
    for id, obj in pairs(self.elementRegistry.Colorpickers) do
        if obj and obj.SetColor then
            obj:SetColor(Color3.fromRGB(255, 255, 255))
        end
    end
    for id, obj in pairs(self.elementRegistry.Dropdowns) do
        if obj and obj.SetValue then
...
function SaveLoadSystem:SetupWithLib(Window, ExistingTab)
    ...
    -- SE NÃO FOI PASSADA UMA TAB EXISTENTE, CRIA UMA NOVA
    if not saveTab then
        saveTab = Window:Tab({Icon = "Settings"}) -- USA UM ÍCONE QUE EXISTE
    end

    -- CRIA A SUBTAB DENTRO DA TAB
    ...
    loadSection:Button(
        "LoadButton",
        {
            ...
            Callback = function()
                ...
                if selected == "No configs found" or selected == "Loading..." then
                    Window:Notification(
                        {
                            Title = "Save & Load",
                            Message = "No configs available!",
                            Duration = 2,
                            Position = "Screen"
                        }
                    )
...
function UIGradient(Color1, Color2, Parent, Rotation, Offset)
    local Gradient
    task.spawn(
        function()
            Gradient =
...
function Lib:GetGui()
    ...
    local parent =
        RunService:IsStudio() and LocalPlayer:FindFirstChild("PlayerGui") or (gethui and gethui()) or
        (cloneref and cloneref(Lib.GetService("CoreGui")) or Lib.GetService("CoreGui"))
...
function Lib:AutoUpdateText(parent)
    parent:GetPropertyChangedSignal("Text"):Connect(
        function()
            task.wait()
            local textSize =
...
function Lib:AutoUpdateList(parent1, parent2, type)
    if type == "Frame" then
        parent1:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(
            function()
...
function Lib:Window(Settings)
    ...
    Left_balls_F.InputBegan:Connect(
        function(input)
    ...
    Left_balls_F.InputChanged:Connect(
        function(input)
            if
    ...
    function StorageTabs:Button(Settings)
        ...
        -- Click effect
        OnClick.MouseButton1Down:Connect(
            function()
                TweenService:Create(
                    Element_Button,
                    TweenInfo.new(0.1),
        ...
        OnClick.MouseButton1Up:Connect(
            function()
                TweenService:Create(
                    Element_Button,
                    TweenInfo.new(0.1),
    ...
    function StorageTabs:Tab(Settings)
        ...
        function Tab:SubTab(Settings)
            ...
            function SubTab:Section(Settings)
                ...
                local ScStroke = CreateElement("UIStroke",{
                    ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                    Thickness = 1,
                    Transparency = 0,
                    Color = Themes.Border.Primary,
                ...
                local PaddingSc = CreateElement("UIPadding",{
                    Parent = Section_Frame,
                ...
                ListLayoutSection:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(
                    function()
                        task.spawn(
                            function()
                ...
                function Section:GetParent()
                    return Section_Frame
                end
                
                function Section:SetStroke(v)
                    ScStroke.Thickness = v and 1 or 0
                end
                
                function Section:SetPadding(vl)
                    if PaddingSc then
                        PaddingSc.PaddingLeft = vl and UDim.new(0, 10) or UDim.new(0, 2)
                        PaddingSc.PaddingRight = vl and UDim.new(0, 10) or UDim.new(0, 2)
                        PaddingSc.PaddingTop = vl and UDim.new(0, 8) or UDim.new(0, 2)
                        PaddingSc.PaddingBottom = vl and UDim.new(0, 8) or UDim.new(0, 2)
                    end
                end
                ...
                function Section:CreateElement(type, properties)
                    properties = properties or {}
                    if not properties.Parent then
                        properties.Parent = Section_Frame
                    end
                    local element = CreateElement(type, properties)
                    return element
                end
                ...
                function Section:Label(Flag, Settings)
                    ...
                    function Label:SetAlignment(alignment)
                        Text.TextXAlignment =
                            alignment == "Center" and Enum.TextXAlignment.Center or
                ...
                function Section:Paragraph(Flag, Settings)
                    ...
                    local Title =
                        ...
                        "TextLabel",
                        {
                            ...
                            TextScaled = false,
                            TextSize = 14,
                            Font = (Paragraph.Settings.Bold.Type == "Title" or Paragraph.Settings.Bold.Type == "Both") and
                                Paragraph.Settings.Bold.Enabled and
                                Enum.Font.GothamBold or
                                Themes.Font.Title,
                            TextWrapped = false,
                            TextXAlignment = Enum.TextXAlignment.Left,
                            TextYAlignment = Enum.TextYAlignment.Center,
                    ...
                    local Text =
                        ...
                        "TextLabel",
                        {
                            ...
                            TextScaled = false,
                            TextSize = 12,
                            Font = (Paragraph.Settings.Bold.Type == "Text" or Paragraph.Settings.Bold.Type == "Both") and
                                Paragraph.Settings.Bold.Enabled and
                                Enum.Font.GothamBold or
                                Themes.Font.Text,
                            TextWrapped = true,
                ...
                function Section:Toggle(Flag, Settings)
                    ...
                    if Flag then
                        ...
                        if SaveLoadSystem and SaveLoadSystem.RegisterToggle then
                            SaveLoadSystem:RegisterToggle(Flag, Toggle)
                        end
                    end
                ...
                function Section:Checkbox(Flag, Settings)
                    ...
                    Checkbox.Settings.Callback = Checkbox.Settings.Callback or function()
                        end

                    pcall(Checkbox.Settings.Callback, Checkbox.Settings.Default)

                    local Element_Checkbox =
                        CreateElement(
                        "Frame",
                        {
    ...
    if Slider.Settings.Style == 1 then
        ...
        return Slider
                ...
                function Section:Dropdown(Flag, Settings)
                    ...
                    Title_F:GetPropertyChangedSignal("AbsoluteSize"):Connect(
                        function()
                            UpdateTitle()
                        end
                    )

                    local Content_Dropdown_F =
                        CreateElement(
                        "Frame",
                        {
                    ...
                    local function CreateDropdownItem(value, index)
                        ...
                        ItemOnClick.MouseButton1Click:Connect(
                            function()
                                if Dropdown.Settings.MultiSelection then
                                    ...
                                    for _, item in pairs(Content_Dropdown_F:GetChildren()) do
                                        if
                                            item:IsA("Frame") and item.Name ~= "UIListLayout" and
                                                item.Name ~= "UIPadding" and
                                                item.Name ~= "UICorner"
                                         then
                    ...
                    if Flag then
                        ...
                        if SaveLoadSystem and SaveLoadSystem.RegisterDropdown then
                            SaveLoadSystem:RegisterDropdown(Flag, Dropdown)
                        end
                    end
    ...
    return TKey
                ...
                function Section:Colorpicker(Flag, Settings)
                    ...
                    local function updateCConteinerColor(input)
                        ...
                        if not Colorpicker_Selector then
                            return
                        end
                        ...
                        if
                            mousePos.X >= selectorPosition.X and mousePos.Y >= selectorPosition.Y and
                                mousePos.X <= selectorPosition.X + selectorSize.X and
                                mousePos.Y <= selectorPosition.Y + selectorSize.Y
                         then
                    ...
                    local function updateColorSlider(input)
                        local mousePos = input.Position
                        if not Colorpicker_RainbowBar then
                            return
                        end
                    ...
                    local function connectSelectorInput()
                        if selectorInputConnection then
                            selectorInputConnection:Disconnect()
                        end
                        if selectorMoveConnection then
                            selectorMoveConnection:Disconnect()
                        end
                        if selectorEndConnection then
                            selectorEndConnection:Disconnect()
                        end

                        selectorInputConnection =
                            Colorpicker_Selector.InputBegan:Connect(
                            function(input)
                                if
                        ...
                        selectorMoveConnection =
                            UserInputService.InputChanged:Connect(
                            function(input)
                                if
                                    draggingCConteiner and
                                        (input.UserInputType == Enum.UserInputType.MouseMovement or
                                            input.UserInputType == Enum.UserInputType.Touch)
                                 then
                                    updateCConteinerColor(input)
                    ...
                    local function connectRainbowInput()
                        if rainbowInputConnection then
                            rainbowInputConnection:Disconnect()
                        end
                        if rainbowMoveConnection then
                            rainbowMoveConnection:Disconnect()
                        end
                        if rainbowEndConnection then
                            rainbowEndConnection:Disconnect()
                        end

                        rainbowInputConnection =
                            Colorpicker_RainbowBar.InputBegan:Connect(
                            function(input)
                                if
                                    input.UserInputType == Enum.UserInputType.MouseButton1 or
                                        input.UserInputType == Enum.UserInputType.Touch
                    ...
                    local closeConnection =
                        ...
                        function(input)
                            if input.UserInputType == Enum.UserInputType.MouseButton1 and Colorpicker.IsOpen then
                                ...
                                local clickedInside =
                                    (mousePos.X >= colorpickerPos.X and
                                    mousePos.X <= colorpickerPos.X + colorpickerSize.X and
                                    mousePos.Y >= colorpickerPos.Y and
                                    mousePos.Y <= colorpickerPos.Y + totalHeight)
                    ...
                    if Flag then
                        ...
                        if SaveLoadSystem and SaveLoadSystem.RegisterColorpicker then
                            SaveLoadSystem:RegisterColorpicker(Flag, Colorpicker)
                        end
                    end
    ...
    function StorageTabs:Notification(Settings)
        ...
        function calculateHeights()
            ...
            local textSize =
                textService:GetTextSize(Settings.Message, 11, Themes.Font.Text, Vector2.new(maxWidth, 10000))
        ...
        function animateIn()
            ...
            for id, notifData in pairs(StorageTabs.NotificationSystem.Notifications) do
                if
                    notifData and notifData.Frame and notifData.Frame.Parent and id ~= notificationId and
                        not notifData.IsClosing
                 then
    ...
    function StorageTabs:SaveLoad(Settings, ExistingTab)
        return SaveLoadSystem:SetupWithLib(StorageTabs, ExistingTab)
    end
...
function Lib:Demo()
    ...
    BasicsSection1:Toggle("ToggleBall", {
        Name = "Ball Toggle",
        Default = false,
        Style = 2,
        Callback = function(value)
    ...
    BasicsSection1:Checkbox("CheckboxDemo", {
        Name = "Checkbox",
        Default = true,
        Callback = function(value)
    ...
    BasicsSection2:Slider("SliderDemo", {
        Name = "Volume",
        Default = 50,
        Minimum = 0,
        Maximum = 100,
    ...
    BasicsSection2:Dropdown("DropdownDemo", {
        Name = "Select Option",
    ...
    AdvancedSection1:Dropdown("MultiDropdownDemo", {
        Name = "Multi Select",
    ...
    AdvancedSection1:Slider("PrecisionSlider", {
    ...
    AdvancedSection2:Slider("DegreesSlider", {
        Name = "Degrees",
        Default = 45,
        Minimum = 0,
        Maximum = 360,
    ...
    TextSection1:Label("LabelSimple", { Text = "Simple Label" })
    TextSection1:Label("LabelBold", { Text = "Bold Label", Bold = true })
    TextSection1:Label("LabelUnderline", { Text = "Underline Label", Subline = true })
    TextSection1:Label("LabelBoth", { Text = "Bold + Underline", Bold = true, Subline = true })
    ...
    InputSection1:Input("NumericInput", {
        Name = "Age",
    ...
    InputSection1:Input("ClearInput", {
        Name = "Clear on Focus",
    ...
    InputSection2:KeyBind("ClickKeybind", {
        Name = "Toggle Menu",
        Default = "K",
        Hold = false,
    ...
    InputSection2:KeyBind("HoldKeybind", {
        Name = "Sprint",
        Default = "LeftShift",
        Hold = true,
    ...
    InputSection2:KeyBind("DefaultKeybind", {
        Name = "Interact",
        Default = "E",
        Hold = false,
        Callback = function(key)
